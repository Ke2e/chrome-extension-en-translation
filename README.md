...
项目采用 **规格驱动开发（SDD）+ OpenAI Codex** 的 AI 结对方式构建：

- **规格先行**：动手写代码前，先在 `docs/` 落盘需求（proposal）、技术设计（design）与任务拆解（task）三份文档，明确每个任务边界与验收标准
- **人机分工**：Codex 按单个任务执行实现，不越界扩展范围；人负责需求澄清、方案审批与逐任务验收
- **小步验收**：每个任务完成后对照验收标准自检，确认通过才进入下一任务，保证代码与需求一致、过程可追溯
