...
> 基于 [需求文档](./proposal.md)、[技术架构文档](./design.md)、[侧边栏布局](./Schematic%20diagram/示意图-弹窗页面.md)、[设置页面布局](./Schematic%20diagram/示意图-设置页面.md) 进行任务拆分。
...
| `src/popup/index.html`   | 侧边栏 HTML 入口（占位）                 |
...
| `src/shared/constants.ts` | 常量定义（DEFAULT\_API\_URL, DEFAULT\_MODEL, STORAGE\_KEYS 等） |
...
**参考示意图**：[示意图-弹窗页面.md §3](file:///e:/ASUS/桌面/SDD/chrome-extension-en-translation/docs/Schematic%20diagram/示意图-弹窗页面.md#L42-L67) - 提取中状态（侧边栏布局）
...
- [ ] 对非文章页面（如视频页、搜索页）返回空结果，供侧边栏展示提示
...
- [ ] 能正确接收并路由 TRANSLATE\_STREAM 消息
- [ ] 支持 Streaming 方式逐块返回翻译结果
- [ ] 翻译结果保留原文 Markdown 格式标记
- [ ] 支持取消翻译（TRANSLATE\_CANCEL 消息）
- [ ] 网络错误 / API Key 无效 / 模型不可用 时返回 TRANSLATE\_ERROR
...
**效果**：存储服务就绪，可在侧边栏打开时恢复上次翻译结果。

***

## 阶段 4：侧边栏 UI（Side Panel）

### 任务 7：侧边栏基础框架与状态管理

**目标**：搭建侧边栏 React 应用，实现 5 种状态的切换管理和 StatusBar 组件。

**涉及文件**：

| 文件                                   | 说明                                                   |
| ------------------------------------ | ---------------------------------------------------- |
| `src/popup/main.tsx`                 | 侧边栏 React 入口                                          |
| `src/popup/App.tsx`                  | 侧边栏主组件，管理 idle/extracting/translating/done/error 五种状态 |
| `src/popup/components/StatusBar.tsx` | 状态栏组件，根据状态显示不同图标和文字                                  |
| `src/popup/styles/popup.css`         | 侧边栏样式                                               |

**五种状态对应的 StatusBar 显示**（参考 [侧边栏页面布局](file:///e:/ASUS/桌面/SDD/chrome-extension-en-translation/docs/Schematic%20diagram/示意图-弹窗页面.md)）：
...
- [ ] 侧边栏打开时显示空状态（idle），中央显示"翻译当前页面"按钮
- [ ] 通过模拟状态切换，能依次看到 extracting → translating → done → error 各状态
- [ ] StatusBar 在每个状态下显示正确的图标和文字
- [ ] 侧边栏高度撑满整个浏览器窗口，宽度由 Chrome 侧边栏决定
- [ ] 底部"设置"入口可点击

**依赖**：任务 1、2

**效果**：侧边栏可正常打开，状态切换可见，UI 框架搭建完成。
...
**效果**：侧边栏具备完整的翻译展示和交互能力，打字机效果动态展示翻译结果。
...
**目标**：将 Side Panel、Content Script、Background 三端串联，跑通完整翻译流程。
...
用户点击"翻译当前页面"
    → Side Panel 发送 EXTRACT_CONTENT → Content Script
    → Content Script 提取 + 转换 → 返回 Markdown 原文
    → Side Panel 显示"提取完成"，发送 TRANSLATE_STREAM → Background
    → Background 发起 Streaming 翻译 → 逐块返回 TRANSLATE_CHUNK
    → Side Panel 逐块更新 md-wx 组件（打字机效果）
    → 翻译完成 → Background 保存结果 → Side Panel 显示"翻译完成"
```
...
| 场景          | 处理方式                              |
| ----------- | --------------------------------- |
| API Key 未配置 | 侧边栏显示"请前往设置页配置 API Key"，提供"前往设置"按钮 |
| API Key 无效  | 侧边栏显示"API Key 无效"，引导用户检查设置         |
| 网络错误        | 侧边栏显示"网络连接失败，请检查网络后重试"             |
| 速率限制        | 侧边栏显示"请求过于频繁，请稍后重试"                |
| 模型不可用       | 侧边栏显示"模型不可用，请检查模型名称配置"             |
| 页面不支持提取     | 侧边栏显示"当前页面无法提取文章内容"                |
| 提取内容为空      | 侧边栏显示"未在页面中找到文章内容"                 |
| 翻译中途取消      | 展示已翻译的部分，不报错                      |
